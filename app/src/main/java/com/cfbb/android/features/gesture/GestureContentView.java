...
public class GestureContentView extends ViewGroup {
    ...
    private int narrowWidth = 140;
    private int narrowPoint = narrowWidth / 2;
    ...
    public GestureContentView(Context context, boolean isVerify, String passWord, GestureDrawline.GestureCallBack callBack) {
        ...
        narrowWidth = getResources().getDimensionPixelSize(R.dimen.guesture_narrowWidth);
        narrowPoint = narrowWidth / 2;

        screenDispaly = PhoneUtils.getScreenDispaly(context);
        blockWidth = (screenDispaly[0] - narrowWidth) / 3;
    ...
    private void addChild() {
        for (int i = 0; i < 9; i++) {
            ...
            image.setLayoutParams(new LinearLayout.LayoutParams(10, 10));
            ...
            int col = i % 3;
            // 定义点的每个属性
            int leftX = col * blockWidth + blockWidth / baseNum + narrowPoint;
            int topY = row * blockWidth + blockWidth / baseNum;
            int rightX = col * blockWidth + blockWidth - blockWidth / baseNum + narrowPoint;
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            int col = i % 3;
            View v = getChildAt(i);
            v.layout(col * blockWidth + blockWidth / baseNum + narrowPoint, row * blockWidth + blockWidth / baseNum,
                    col * blockWidth + blockWidth - blockWidth / baseNum + narrowPoint, row * blockWidth + blockWidth - blockWidth / baseNum);
        }
    }
